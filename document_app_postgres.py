...
DB_NAME = "pgaidatabase"  # Extract from connection string or use default
...
# Initialize PGVector with embeddings
try:
    ...
    logger.info(f"Successfully connected to PostgreSQL vector database")
    
    # Add documents to the vector store (if any)
    if docs:
        vectorstore.add_documents(docs)
        logger.info(f"Successfully added {len(docs)} document chunks to PostgreSQL vector store")
    else:
...
# The retriever will use your PostgreSQL vector store to find relevant chunks of text based on the user's query.
...
print("PostgreSQL-based Document Q&A System Ready")
